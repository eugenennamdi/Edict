...
- **Stop:** Approval challenge/signing/submission UI composition, approval persistence from the UI, execution, and live actions are not authorized by this checkpoint. The production `ApprovalGateway` remains dormant. Account-backed compatibility evidence remains separate work for the authorized Brickken account holder or administrator.

## Run planning workspace

The planning transport uses only `POST /api/runs`, `GET /api/runs/[runId]`, and `POST /api/runs/[runId]/cancel`. The existing five-route production inventory is unchanged. The approval-readiness UI performs only local injected-wallet discovery and explicit account/chain readiness actions; approval challenge/submission and external-effect controls are absent. Server normalization, plans and hashes remain authoritative. Browser validation provides input feedback without generating identities or hashes.
