...
- **Stop:** Transaction preparation, `eth_sendTransaction`, Brickken/RPC execution, confirmation, polling, read-back, receipts, and other live actions remain unauthorized. Account-backed wallet compatibility evidence remains separate work for the authorized operator.

## Run planning workspace

The planning transport uses `POST /api/runs`, `GET /api/runs/[runId]`, and `POST /api/runs/[runId]/cancel`; explicit approval additionally uses the existing challenge and approval routes. The production inventory remains exactly five `/api/runs` routes. The approval UI performs no transaction execution or external write. Server normalization, plans, hashes, challenge validation, signer recovery, CAS persistence, and the final durable approval state remain authoritative.
