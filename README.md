...
- **Durable recovery checkpoint:** The root page remains the new-mandate surface and `/runs/[runId]` is the canonical recorded-run surface. Create and authorized GET return the same strict complete planning record. Reload and same-browser tabs reconstruct from durable server state without changing revision. The run ID is only a locator; the existing one-active-run HttpOnly capability remains the authority.
- **Stop:** Approval UI/composition, execution, and live actions are not authorized by this checkpoint. Account-backed compatibility evidence remains separate work for the authorized Brickken account holder or administrator.
