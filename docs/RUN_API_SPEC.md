...
**SUPERSEDING PRODUCT DECISION — 2026-09-13** — The narrow routes above remain internal/diagnostic compatibility surfaces, but the normal browser flow uses `POST /api/runs/[runId]/execute`. The request contains only `expectedRevision`; it composes the same explicit CAS transitions and returns the strict one-use send-authorized envelope. It performs at most one Brickken preparation dispatch per persisted intent and at most one legal stale reprepare. It never retries an ambiguous preparation. The browser automatically invokes `/track` with bounded backoff after durable hash ingestion; the optional record refresh remains read-only.
...
**DECISION** — The server re-reads the durable run and permits only an approved, nonterminal `TOKENIZATION/PREPARING` run whose first operation is `TOKENIZE/NOT_STARTED`. One `BEGIN_PREPARE` CAS winner advances revision `N→N+1`, the write gate is checked again, one `newTokenization` prepare-only request is made with `execute:false` and `client-broadcast`, and its outcome is persisted by a second CAS at `N+2`. `tokenizerEmail` comes only from validated server-only `BRICKKEN_TOKENIZER_EMAIL`; the manifest project/issuer email cannot change Brickken account identity. A valid result becomes `PREPARED`; an indeterminate or malformed result becomes `PREPARE_UNKNOWN/RECONCILIATION_REQUIRED`; a definite upstream refusal becomes `REJECTED/FAILED` with terminal outcome `FAILED`. Failures persist and return only one bounded Edict category (`AUTHENTICATION_REJECTED`, `ENTITLEMENT_REJECTED`, `CREDITS_EXHAUSTED`, `INVALID_REQUEST`, `SIGNER_NOT_APPROVED`, `UPSTREAM_RATE_LIMITED`, `UPSTREAM_SERVER_ERROR`, `PREPARATION_REFUSED`, or `PREPARATION_UNCONFIRMED`). A stale or concurrent request is `409 REVISION_CONFLICT`; an ineligible state is `409 STATE_CONFLICT`; disabled preparation is `404 PREPARATION_DISABLED`. Raw upstream bodies, prose, payloads, credentials, and stacks are never persisted or returned.

**DECISION** — The public `execution` projection is strict and server-derived. It reports the exact TOKENIZE preparation state as `READY_FOR_PREPARATION`, `PREPARATION_PENDING`, `PREPARED_FOR_REVIEW`, `PREPARATION_UNCONFIRMED`, or terminal `PREPARATION_FAILED`. Only a durably prepared operation includes `PreparedTransactionReviewV1`, binding run/revision, manifest and plan hashes, approval revision, operation, signer, sandbox/Sepolia, Brickken action/mode, prepared ID, normalized exact wallet request, chain precondition, and a canonical SHA-256 fingerprint. A failed projection contains no transaction review and cannot expose prepare or promotion actions. Already-durable legacy `FAILED + PREPARE_UNKNOWN` records are projected as `PREPARATION_FAILED`; they are not rewritten. The projection omits the persisted raw unsigned object, raw Brickken response, proof/events/observations, credentials and capabilities.
...
**HISTORICAL DECISION** — The preparation milestone stopped before wallet execution. It has been superseded by the TOKENIZE-only Execute Mandate, broadcast-result and automatic tracking routes. WHITELIST, MINT and the full deployment receipt remain disabled.

**DECISION — current product command** — `POST /api/runs/[runId]/execute` accepts only `expectedRevision`. The server re-reads the run, rejects plans outside the TOKENIZE-only capability matrix, performs at most one legal durable replacement, and reruns all semantic and external readiness checks before its final authority CAS. `POST /track` reserves one of 30 durable tracking slots before any read and cannot prepare, authorize, prompt, send or replace a transaction.
...
**CONTROLLED OBSERVATION — 2026-09-09** — A preparation-only diagnostic received HTTP 400 with license/subscription/entitlement semantics and no prepared identifier or transaction. This observation is separate from historical run `e0979e24-ad1e-432c-978c-d9f5bd9aefee`, whose stored record does not prove dispatch or response details. Neither diagnostic reached a wallet or blockchain operation. Current code exposes gated TOKENIZE execution and tracking; live signer/license/credit, named-wallet and end-to-end completion evidence remains outstanding.
