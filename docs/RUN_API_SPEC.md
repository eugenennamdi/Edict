...
**DECISION — status/read-back cleanup, 2026-09-13** — `/track` may retain bounded `/transaction-status` text, but no raw string authorizes failure, success, or phase advancement. TOKENIZE identity remains unavailable because the current symbol-scoped token/tokenizer reads cannot prove that their address was created by this run's finalized transaction; the bounded server error is `READ_BACK_BINDING_UNRESOLVED` and the durable run remains in TOKENIZATION.
