...
**LIVE READ-ONLY OBSERVATION — 2026-09-13** — Run `e0979e24-ad1e-432c-978c-d9f5bd9aefee` is immutable V2 revision 4, `TOKENIZATION/FAILED`, terminal `FAILED`, with TOKENIZE stage `PREPARE_UNKNOWN`, no prepared `txId`, no unsigned transaction and events `BEGIN_PREPARE` then `RECORD_PREPARE_FAILURE`. The event proves the adapter classified a definite refusal, not a successful or uncertain response, but the exact sanitized refusal code was not persisted and cannot be reconstructed without guessing. The legacy stage name was misleading: new definite refusals use `REJECTED`, while the public projector recognizes this already-durable terminal combination as `PREPARATION_FAILED` without modifying it.
