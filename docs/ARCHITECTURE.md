...
**DECISION** — The vendor-neutral browser boundary performs passive EIP-6963 discovery, explicit EIP-1193 provider selection, server-issued EIP-712 approval, strict transaction projection, and V4 release-before-send/hash-handoff ordering. A server-owned capability matrix enables TOKENIZE and disables WHITELIST/MINT. The private global execution flag is an operator emergency kill switch. [`WALLET_EXECUTION_SPEC.md`](WALLET_EXECUTION_SPEC.md) owns the detailed wallet contract.
