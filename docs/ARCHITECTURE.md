...
**DECISION** — The vendor-neutral browser boundary performs passive EIP-6963 discovery, explicit EIP-1193 provider selection, server-issued EIP-712 approval, strict transaction projection, and V4 release-before-send/hash-handoff ordering. A server-owned capability matrix enables TOKENIZE, and enables WHITELIST and MINT only when the approved mandate includes one investor allocation. The private global execution flag is an operator emergency kill switch. [`WALLET_EXECUTION_SPEC.md`](WALLET_EXECUTION_SPEC.md) owns the detailed wallet contract.
...
**DECISION — Checkpoint 3 TOKENIZE semantic activation preparation, 2026-09-12** — Production runtime now selects the real `TokenizeOnlySemanticAuthorizationEvaluator` only when private `EDICT_TOKENIZE_EXECUTION_ENABLED` is exactly `1` and the exact destination, canonical function signature and exact calldata commitment are all valid. Otherwise it composes `DenyAllSemanticAuthorizationEvaluator`. No environment or development mode enables it implicitly, and any `NEXT_PUBLIC_EDICT_TOKENIZE_*` variable is refused. The local gate and policy values remain empty.

**DECISION** — The TOKENIZE evaluator revalidates the current V4 durable run only: approval proof identity, canonical manifest hash, rederived deterministic plan hash, active operation/attempt, txId, recomputed preparation fingerprint, Sepolia signer and six immutable fields, exact `FeeAuthorizationV1`, prepared-review identity, configured destination, signature-derived selector, calldata commitment, `newTokenization`, and `client-broadcast`. Its authorization ID hashes the current run revision and all of those bindings. It hard-denies WHITELIST, MINT, and unknown operations. Repository CAS remains the final authority boundary, so a concurrent revision or repreparation invalidates the decision.

**DECISION — server-owned fee headroom** — Brickken fee fields are persisted as wallet-request defaults. For every newly created V4 preparation attempt, Edict separately derives fixed relative and absolute EIP-1559 maxima on the server, persists them in `FeeAuthorizationV1`, includes them in the preparation fingerprint and wallet-intent hash, and checks trusted current base fee and balance before releasing provider authority. Neither the browser nor the wallet can choose or expand this envelope. Old exact-cap authorizations are decoded without expansion, and post-broadcast evaluation always uses the historical envelope bound to that attempt.
...
**DECISION — transaction-bound TOKENIZE identity, 2026-09-13** — `/transaction-status` text remains opaque structural evidence with no lifecycle authority. After exact durable `txId + txHash` correlation, trusted-RPC six-field identity match, fee compliance, a successful canonical receipt and Ethereum `FINALIZED` head, Edict re-reads that exact receipt and verifies the factory proxy's ERC-1967 implementation at the receipt block. Exactly one `NewTokenization(uint256 indexed id,address indexed token,address indexed escrow)` log must then be emitted by `0x23b04b6410d72fa66a77a9e0146df6634ad4c462`; topic0 is independently derived as `0xf3891eaa15d438132be54582fa326e471227aacade9ebed32006b0ba9d63254c`. The decoded token and escrow must be nonzero, distinct and structurally canonical. `/get-token-info` and `/get-tokenizer-info` are secondary confirmation only: the latter must equal the already-derived token address while signer, chain and manifest metadata match. Only then does the server persist `TokenIdentityV1` and its evidence commitment. Caller-supplied identity remains categorically rejected.
...
**DECISION — operator calldata review** — `npm run review:tokenize-calldata -- <run-id>` loads ignored `.env.local`, performs only a durable `getById`, and accepts canonical V2 or V4 `PREPARED` TOKENIZE state. Success emits only run ID, schema version, revision, prepared `txId`, destination, four-byte selector and `sha256:` commitment of the exact canonical calldata. Failure emits one bounded Edict-owned reason to stderr and a nonzero exit code; caught database/upstream detail is never printed. It neither changes state nor reads or changes the activation gate.

**LIVE READ-ONLY OBSERVATION — 2026-09-13** — An explicitly authorized authenticated sandbox `GET /get-network-info?chainId=11155111` returned HTTP 200 with string fields `currencyName`, `blockExplorerUrl`, `factoryAddress`, `BKNAddress`, `USDTAddress`, and `USDCAddress`; the sanitized factory was `0x23B04b6410D72Fa66A77a9e0146DF6634Ad4C462`. The prior strict schema omitted the last four address fields and therefore rejected the otherwise valid response. The corrected schema validates every observed field but the server-only preflight exposes only `factoryAddress`. A trusted Sepolia read of ERC-1967 slot `0x360894a13ba1a3210667c828492db98dca3e2076cc3735a920a3ca505d382bbc` at `finalized` resolved `0x2C24f3Fe7665eA83B2280bb5a7E66072C869AD89`, matching the independently reviewed implementation. Production policy now accepts only the exact bare canonical signature `newTokenization((string,string,string,uint224,address,address,bool,uint256[],address[]),(address,uint256,address,address,uint256,uint256,bytes),(uint256,address,address,uint256,uint8,bytes32,bytes32))` and independently derives `0xf3d02cfd`. The local gate remains off and no live signature configuration was changed.
...
**LIVE READ-ONLY OBSERVATION — corrected 2026-09-14** — Historical run `e0979e24-ad1e-432c-978c-d9f5bd9aefee` remains immutable V2 revision 4, `TOKENIZATION/FAILED`, terminal `FAILED`, with TOKENIZE stage `PREPARE_UNKNOWN`, no prepared `txId`, no unsigned transaction and events `BEGIN_PREPARE` then `RECORD_PREPARE_FAILURE`. Edict durably recorded a terminal preparation failure and never reached the wallet/broadcast path. The record does not establish whether dispatch occurred, whether an HTTP response arrived, or the exact failure code; it therefore does not prove that Brickken refused the request. It must not be retried or mutated.

## Product workflow and internal engine

**DECISION — final productization, 2026-09-13** — The normal user workflow is **Mandate → Plan → Approve → Execute → Wallet → Verified result**. A normal Edict user never edits configuration. The user-facing plan contains four meaningful outcomes—create the asset, configure investor access, issue the allocation, and verify completion—while the immutable seven-operation plan remains the internal security ledger.

**DECISION** — `POST /api/runs/[runId]/execute` is the single TOKENIZE product command. With the submitted expected revision it may perform the existing explicit CAS preparation, V4 promotion, freshness evaluation, one legal pre-authority reprepare, semantic authorization, and release-before-send transitions. It does not weaken or bypass any transition. Ambiguous preparation, released wallet authority, or an existing hash categorically prevents preparation dispatch.

**DECISION** — The calldata commitment is computed from the exact normalized prepared calldata by the server. New V4 preparation attempts persist it, include it in the attempt fingerprint, expose it in the prepared-review integrity record, and bind it into the wallet intent. Historical V4 attempts remain readable; their immutable calldata was already included in their original preparation fingerprint, and their commitment is recomputed before authority release. No browser or environment value supplies this commitment.

**DECISION** — After durable hash ingestion the client performs bounded, backoff-based calls to the existing read-only tracking orchestration. Tracking never reopens a wallet, prepares, sends, resends, or replaces. Raw durable stages appear only in expandable technical details; the primary projection uses Ready to execute, Verifying, Completed, Needs attention, and Failed.

### Operator emergency controls

`EDICT_TOKENIZE_EXECUTION_ENABLED` is a deployment-wide emergency kill switch, not per-transaction authorization. Static destination, selector, Brickken sandbox, RPC, database, origin, security secret, price-buffer, and private fee-policy roots remain operator-owned. No run-specific value requires an environment change or application restart.
...
3. **DECISION** — Server decodes the persisted TOKENIZE calldata with the reviewed ABI, reconstructs it from approved mandate fields plus independently validated protocol fields, requires byte equality, and binds the result into the canonical wallet intent. Immediately before the final authority CAS, it reruns plan/approval/preparation identity, factory implementation, call simulation, report lifetime and signer, chain, nonce, balance and base-fee checks. The browser verifies the released operation identity, signer, chain and prompt revision.
