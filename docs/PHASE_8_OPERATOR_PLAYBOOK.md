...
There is no environment spread, environment dump, root environment-file loading or browser delivery. The `BRICKKEN_READ` runner copies only the listed non-secret harness settings and `BRICKKEN_API_KEY`, hard-codes its action and sandbox mode, and accepts no command arguments. It constructs neither the adapter nor its transport until the single-use Execute grant has passed and the executor has revalidated its narrowed configuration. API keys must be non-empty bounded visible ASCII without controls, CR/LF or surrounding whitespace. One centralized safe writer owns every CLI-controlled stdout/stderr write. It collects the narrowed credential first, preflights every fixed output before bootstrap generation or construction, and checks the exact final string immediately before either terminal sink. A colliding error fallback is silent and nonzero. Startup also fails if any configured credential collides wholly or by containment with browser-visible target, origin, HTML or evidence metadata, including short values.
...
The child build inherits no ambient environment. Its explicit allowlist contains only production/build controls and a synthetic `BRICKKEN_API_KEY`; an audit-owned operating-system sandbox profile denies all network activity without modifying Node or framework globals. A fresh Turbopack build is attempted first; only the known sandbox process/port restriction permits deletion of that attempt's isolated output and a fresh webpack fallback. The audit requires a nonempty build ID, build manifests, parseable route and App Router manifests, the exact root page plus five approved API routes, and at least one nonempty client JavaScript artifact. It separately scans browser-delivered `.next/static` files, prerendered response artifacts and tracked `public/` files; server-only JavaScript output is not misclassified as a client bundle.

Success reports `HEAD`, the copied-source digest, the emitted-artifact digest, the builder and the five routes only after validation and cleanup. Missing, empty, stale, malformed, incomplete, mismatched or leaking output fails closed. The repository's existing `.next` directory is neither copied nor accepted as evidence.
