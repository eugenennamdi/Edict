...
Success reports `HEAD`, the authoritative copied-source digest, the emitted-artifact digest, the builder and the nine routes only after validation and cleanup. Missing, empty, stale, malformed, incomplete, mismatched or leaking output fails closed. Source revision/digest binding is deterministic; emitted artifact digests are evidence identifiers and are not expected to be reproducible because Next build metadata may vary. The repository's existing `.next` directory is neither copied nor accepted as evidence.
...
**HISTORICAL OPEN QUESTION — Phase 8** — At this checkpoint no authenticated Brickken write had occurred. Phase 10C later performed controlled preparation-only diagnostics that produced no transaction. Signer approval, active tokenizer licensing, credits, prepared write payloads, selected-wallet compatibility, provider chain behavior, RPC transaction equivalence, receipts, finality, Brickken confirmation/status and post-write behavior remain unverified. Migration `0002_gorgeous_squadron_sinister.sql` still requires separate authorization before application.
