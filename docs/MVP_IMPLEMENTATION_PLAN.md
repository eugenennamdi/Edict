...
**DECISION** — Phase 7 should implement a browser-only Sepolia wallet adapter with injected EIP-1193 behavior, exact pre-sign comparison of the persisted prepared transaction against the approved operation, explicit confirmation per transaction and durable hash handoff. Keep Brickken writes disabled and do not add public external-effect routes until these safety tests pass.
