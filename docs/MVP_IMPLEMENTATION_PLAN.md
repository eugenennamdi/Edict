...
**DECISION** — Phase 8 should perform an adversarial review of the Phase 7 ambiguity boundary and specify the smallest explicitly authorized compatibility harness for one selected wallet/version across tokenization, whitelist, and mint. Keep semantic authorization deny-all, Brickken writes disabled, and external-effect routes absent until the review and human-authorized sandbox prerequisites are satisfied.
