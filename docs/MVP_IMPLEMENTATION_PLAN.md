...
## Browser wallet composition behind production deny-all

**DECISION — 2026-09-12** — The V4 authority model is composed to the browser through strict wallet-authorization, broadcast-hash, and broadcast-unknown routes. Authority release durably sets `INVOKED_OR_UNKNOWN` before returning the minimal exact-request envelope. The browser reuses explicit EIP-6963 selection, account-at-any-index matching, Sepolia switching, generation invalidation, and bounded provider calls; it invokes `eth_sendTransaction` at most once and treats every post-release failure as ambiguous. Production runtime retains the default deny-all semantic evaluator. Tests use fake providers and explicit injected allow behavior only. No schema change, migration, live wallet transaction, Sepolia broadcast, or live Brickken call is part of this checkpoint.

**DECISION — 2026-09-12 hardening** — The last pre-send sequence is generation → chain → generation → accounts → generation/readiness → generation → the sole captured-provider send invocation, whose closure performs the final adjacent generation assertion. Typed gateway errors distinguish deny-all, revision conflict, authorization-response uncertainty, malformed response and sanitized rejection without retry. Durable refresh locks stale execution UI after authorization uncertainty/conflict or post-authority ambiguity. The unused legacy send coordinator/barrel export is removed, and repository tests assert exactly one production send site plus direct-PREPARED CAS concurrency, response-loss durability and stale-intent rejection after repreparation.

**DECISION — Checkpoint 3 activation preparation, 2026-09-12** — A TOKENIZE-only semantic evaluator is implemented behind exact private `EDICT_TOKENIZE_EXECUTION_ENABLED=1` plus strict destination, canonical function-signature and calldata-commitment policy. Default and incomplete configurations remain `DenyAllSemanticAuthorizationEvaluator`; WHITELIST/MINT are hard-denied. The repository has no authoritative destination/ABI trust root, so local values remain empty and final activation is blocked pending operator-reviewed evidence. No live call, wallet action, Brickken write, Neon mutation, migration, or transaction is part of this checkpoint.
