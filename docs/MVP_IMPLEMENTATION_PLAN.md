...
**DECISION** — Phase 6 first completes Task 6's current-official-documentation discrepancy checklist, then implements Task 7's same-origin run APIs and orchestration guards against the verified durable repository using injected transports. Do not implement wallet code, polling jobs, receipts, or live Brickken writes in that phase.
