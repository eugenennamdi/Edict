...
**DECISION — Current TOKENIZE execution policy, 2026-09-14** — The canonical record UI composes EIP-6963 discovery and `SelectedWalletSession` with a V4-only HTTP gateway and one-shot coordinator. The browser never constructs semantic or transaction authority. The product `POST /execute` accepts only the displayed revision. The server-owned capability matrix enables TOKENIZE, and enables WHITELIST/MINT for approved one-investor mandates; `EDICT_TOKENIZE_EXECUTION_ENABLED` is the operator emergency kill switch. Every envelope is released only after current semantic and external-readiness evidence is bound by the final durable CAS. WHITELIST and MINT reuse the same prepare → client-broadcast → `/send-transactions` correlation → receipt/finality → Brickken read-back path. Destination for those operations is the verified token address from TOKENIZE read-back, not the factory. Plan-hash re-derivation remains mandatory on every mutation.
...
**SUPERSEDING PRODUCT DECISION — 2026-09-13** — Promotion, preparation, readiness, authorization, and tracking remain distinct durable security transitions but are no longer separate primary controls. One **Execute mandate** action validates the selected provider session, invokes the product execute command, and consumes exactly one returned envelope at the sole production `eth_sendTransaction` call site. Account access and Sepolia switching occur only when the wallet actually requires them. Hash ingestion locks execution; subsequent bounded tracking is automatic and read-only with respect to wallet and chain submission.

**DECISION** — `EDICT_TOKENIZE_EXECUTION_ENABLED` is only an emergency operator kill switch. Exact calldata commitment is server-derived and attempt-bound; no user or operator copies a transaction hash into configuration. Fee defaults remain Brickken suggestions, while the private server policy persists immutable `SERVER_BOUNDED_HEADROOM` caps before wallet authority release.

**DECISION — preparation-state recovery, revised 2026-09-14** — V2 `PREPARED` remains readable, while the product-level Execute Mandate command internalizes V4 promotion, current readiness checks and authority release. No manual promote, prepare, readiness or tracking control appears in the normal UI. `PREPARE_UNKNOWN/RECONCILIATION_REQUIRED` remains an unresolved, no-retry state. A definite terminal failure displays `PREPARATION_FAILED`. Refresh and lost-response recovery derive these outcomes only from durable state.

**DECISION — durable preparation bound** — One TOKENIZE operation may contain exactly one initial preparation and at most one pre-authority replacement. The preparation-attempt array is the durable counter. A repository revision CAS permits only one concurrent replacement winner. If attempt two becomes stale, Execute Mandate fails closed and requires a new run; attempt three is illegal.

**DECISION — resumed authority freshness** — `WALLET_PROMPT_RECORDED` does not preserve external readiness. Both fresh and resumed executions rerun current chain, nonce, balance, base fee, report deadline/safety buffer, canonical calldata, report signer/source/domain, factory, implementation and exact read-only simulation checks immediately before the final authority CAS. The refreshed semantic authorization and nonce evidence are persisted in the same winning CAS that changes provider invocation to `INVOKED_OR_UNKNOWN`.

**DECISION — automatic tracking bound** — Each automatic tracking request first reserves a durable `TRACK_EXECUTION_RESERVED` event with repository CAS. The server permits at most 30 reservations per run and enforces a two-second minimum interval. The reservation happens before RPC or Brickken reads, so reloads, remounts, revision changes, missing transactions, transport failures and concurrent calls cannot reset or bypass the bound. Tracking can only reconcile the existing hash; it never prepares, prompts, sends, replaces or changes transaction identity.
...
**HISTORICAL DECISION** — Phase 7 completed the offline, vendor-neutral browser primitives before production TOKENIZE authorization was added.
...
That historical route inventory has been superseded by Execute Mandate, broadcast result ingestion, and automatic tracking routes.
...
**DECISION** — Exact Brickken-prepared gas values remain hash-bound transaction defaults, not the fee ceiling. Newly prepared V4 attempts persist a server-owned `SERVER_BOUNDED_HEADROOM` envelope before wallet authority release: gas limit is capped at the lesser of 120% of the prepared limit and 8,000,000; max fee is capped at the lesser of the greater of 300% of the prepared value or 3 gwei and 10 gwei; priority fee is capped at the lesser of the greater of 300% of the prepared value or 2 gwei, 3 gwei, and the max-fee cap; and total authorized network fee is the exact gas-cap × max-fee-cap product with an absolute 0.1 ETH ceiling. A prepared value already outside those ceilings is refused. The trusted pre-release freshness check still requires the current base fee to fit the persisted max-fee cap and sufficient balance for value plus the entire persisted network-fee cap. Browser/API input cannot select or expand caps. Historical `BOUNDED_NO_INCREASE` envelopes remain valid and unchanged.

**DECISION** — Fee-model classification is owned by the transaction `type` and corresponding EIP-1559 fields. A mined type-`0x2` RPC response may contain `gasPrice` as its effective gas price; that compatibility field does not make it a legacy transaction. Type `0x0` is legacy, while any other change away from the authorized type is a fee-model change. For type `0x2`, actual gas limit, max fee, priority fee, access list, and exact gas × max-fee product must remain within the already-persisted server envelope. `FeeAuthorizationV1` is an authorization and audit boundary; it cannot physically control an external wallet after provider invocation.
...
**DECISION** — `PreparedTransactionReviewV1` is a non-authorizing display envelope. Wallet authority additionally requires canonical semantic decoding and re-encoding of the exact `newTokenization` call. URL, name, symbol, 18-decimal supply cap, issuer/payer, empty pre-mints and empty initial holders are derived from the approved durable mandate. Payment-token/oracle configuration, signed fee report and optional permit are preserved from Brickken only after read-only factory validation, deadline and nonce checks, contract-code checks, reviewed implementation verification and exact-call simulation. The resulting bytes must equal Brickken's normalized calldata before their SHA-256 commitment is persisted.
