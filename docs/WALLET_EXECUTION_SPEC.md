...
**DECISION — final activation boundary, corrected 2026-09-13** — The minimum product composition is explicitly ordered: promote V2 PREPARED, check V4 readiness, select/check the wallet, authorize/send once, then invoke tracking. No step triggers the next automatically. Production V4 tracking calls the existing exact-pair correlation adapter and hardened status adapter; it never uses the legacy confirmation alias. Transaction-status text is opaque evidence and cannot advance lifecycle. Current server-owned token/tokenizer reads can validate only a symbol-scoped candidate, not bind its address to this finalized transaction, so they always stop with `READ_BACK_BINDING_UNRESOLVED`; callers cannot submit an identity and WHITELIST remains unreachable.
