...
**DECISION — Current TOKENIZE execution policy, 2026-09-14** — The canonical record UI composes EIP-6963 discovery and `SelectedWalletSession` with a V4-only HTTP gateway and one-shot coordinator. The browser never constructs semantic or transaction authority. The product `POST /execute` accepts only the displayed revision. The server-owned capability matrix enables TOKENIZE and disables WHITELIST/MINT; `EDICT_TOKENIZE_EXECUTION_ENABLED` is the operator emergency kill switch. Every envelope is released only after current semantic and external-readiness evidence is bound by the final durable CAS.
