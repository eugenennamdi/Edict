...
**DECISION — preparation-state recovery, 2026-09-13** — V2 `PREPARED` is a complete review state and does not require promotion merely to render its prepared transaction. The product first displays that review and a separate `Promote to execution state` action; only explicit promotion produces V4 `PREPARED`, after which readiness is separately invoked. `PREPARE_UNKNOWN/RECONCILIATION_REQUIRED` displays an unresolved, no-retry state. A definite refusal displays terminal `PREPARATION_FAILED`, with no prepare, promotion, readiness or wallet action. Refresh and lost-response recovery derive these outcomes only from the durable projection.
