...
**INDEPENDENT EXPLORER EVIDENCE — 2026-09-13** — The fully verified, unchanged-bytecode Sepolia source for implementation `0x2C24f3Fe7665eA83B2280bb5a7E66072C869AD89` identifies `STOFactoryManagedUpgradeable`. Its `newTokenization` calls `_newTokenization`; that base function increments `idSTOs`, deploys and stores `stoTokens[idSTOs]` and `stoEscrows[idSTOs]`, and returns both. The wrapper then completes token minter setup and emits `NewTokenization(idSTOs, stoTokens[idSTOs], stoEscrows[idSTOs])`. ABI: `event NewTokenization(uint256 indexed id,address indexed token,address indexed escrow)`. [Blockscout verified implementation source and ABI](https://eth-sepolia.blockscout.com/api/v2/smart-contracts/0x2C24f3Fe7665eA83B2280bb5a7E66072C869AD89)

**INDEPENDENT SOURCE TRACE — event inventory** — The factory directly emits conditional `ChargeFee(address indexed issuer,address token,uint256 amount)` during `_chargeFee` and mandatory `NewTokenization` after both proxy deployments and minter-role setup succeed. The custom `BeaconProxy` constructor deliberately writes admin/beacon slots without emitting their ERC-1967 events, but its initializer delegatecalls can emit token/escrow `Initialized`, access-control events and premint `Transfer` events. The later token `addMinter`/`renounceRole` calls can emit role events. Fee-token `permit`/`transferFrom`, proxy initializers and other external calls can emit ERC-20 or contract-specific logs, so their total event vocabulary is not bounded by the factory ABI. Edict therefore trusts no event name from those emitters: it allowlists only the exact `NewTokenization` topic emitted from the reviewed factory proxy and requires exactly one occurrence. A reentrant or otherwise abnormal path producing two is refused.

**DECISION — production TOKENIZE receipt binding, 2026-09-13** — The V4 orchestrator re-reads only the durable transaction hash, proves the same canonical finalized inclusion, proves the proxy implementation at that receipt block, and accepts exactly one reviewed-factory `NewTokenization` log with the independently derived topic. The event supplies the authoritative token address; symbol-scoped Brickken reads cannot select or replace it. `/get-tokenizer-info` must return the identical address and all secondary signer/chain/metadata checks must pass before `TokenIdentityV1` is persisted. This resolves `READ_BACK_BINDING_UNRESOLVED` for the reviewed implementation while retaining fail-closed behavior for missing, malformed, duplicate, decoy or implementation-mismatched evidence.
